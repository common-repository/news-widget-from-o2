<?php
/*
  Plugin Name: News Widget from O2
...
$options = array(
  ...
  if($options['cache_content'] == 'yes') {
	  ...
	  if($current_date > $cache_date) {
          ...
          curl_setopt($o2_feed, CURLOPT_USERAGENT, 'O2 WordPress News Widget');
...
if ($options['aff_id']) { $affinfo = 'http://www.awin1.com/cread.php?awinmid=3235&awinaffid='.$options['aff_id'].'&clickref=&p='; } else { $affinfo = ''; }
...
$options = array(
  ...
  if (!$options['title_chars'] || $options['title_chars'] == 0) {
  	$options['title_chars']		=	'60';
  }
  if (!$options['desc_chars'] || $options['desc_chars'] == 0) {
  	$options['desc_chars']		=	'120';
  }
  
?>
	  <p>
    <label>Affiliate Window ID: <br /></label><input type="text" id="aff_id" name="aff_id" value="<?php echo $options['aff_id'];?>"><br />
